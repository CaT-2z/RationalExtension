...
public class RootOfUnityBasisPart implements IBasisPart{
    ...
    public Rational addAndRemainder(Rational e){
        ...
        if(div[0].compareTo(BigInteger.ZERO) != -1 && div[1].compareTo(BigInteger.ZERO) != -1){
            return new Rational(BigInteger.valueOf(-1).pow(div[0].intValue()), BigInteger.ONE);
        }else{
            value.numerator = value.numerator.add(value.denominator);
            value.multiply(Rational.HALF);
            return new Rational(BigInteger.ONE, BigInteger.valueOf(-1).pow(div[0].abs().add(BigInteger.ONE).intValue()));
        }
    }
